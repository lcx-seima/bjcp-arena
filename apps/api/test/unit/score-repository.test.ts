...
describe("prisma score repository", () => {
  ...
  it("does not query Prisma when listing scores for an empty beer list", async () => {
    ...
    await expect(repository.listScoresForBeers([])).resolves.toEqual([]);
    expect(findMany).not.toHaveBeenCalled();
  });

  it("maps nullable public score fields from Prisma records", async () => {
    ...
    await expect(repository.listScoresForBeers([2, 4])).resolves.toEqual([
