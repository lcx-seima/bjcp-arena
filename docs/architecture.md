...
本仓库是啤酒评鉴系统的 pnpm monorepo。当前目标是搭好可联通的工程底座：一个 API、两个前端入口、共享契约，以及共享 API 客户端。
...
```text
admin/judge -> @bjcp-arena/api-client -> http://<api-host>:4000/api/*
```
...
当出现评鉴会、裁判、酒款、评分等具体工作流时，再扩展领域模型和业务表。当大屏重新进入范围时，再重新引入对应前端、契约、接口和实时传输通道。
